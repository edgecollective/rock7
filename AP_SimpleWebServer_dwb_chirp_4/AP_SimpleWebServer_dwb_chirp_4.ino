        ...
        while (client.connected()) {
                ...
                if (c == '\n' && currentLineIsBlank) {
                    ...
                    if (StrContains(HTTP_req, "ajax_inputs")) {
                        ...
                        if (GetText(txt_buf, TXT_BUF_SZ)) { /// this strips the incoming text of extraneous stuff
                          Serial.println("\r\Message Text:");
                          Serial.println(txt_buf);
                          
                          //chirpText(txt_buf,TXT_BUF_SZ); /// our satellite function

                          Serial.print("string length=");
                          Serial.println(strlen(txt_buf));
                          
                          char str[] = "AT+SBDWT=";
                          strcpy(out_str,str);
                          
                          Serial.print("Full Rock7 string=");
                          strcat(out_str,txt_buf);
                          Serial.println(out_str);
                          sendCheckReply("AT&K0","OK",timer);
                          sendCheckReply(out_str,"OK",timer);
           ...
           // delay(5000);
            ...
            if (level>=threshold) {
              ...
              client.print("<br>Insufficient signal to broadcast.<br> Reload page to reevaluate signal.");
